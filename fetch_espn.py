...
HEADERS = {
    ...
    "Referer": "https://www.espn.com/fantasy/baseball/",
    "Origin": "https://www.espn.com",
    "x-fantasy-source": "kona",
    "x-fantasy-platform": "kona-PROD-b80a0c843e6f7d6e3e24610cee8e1d09beeb9d9e",
}
...
def fetch(*views):
    ...
    cookies = COOKIES if (ESPN_S2 and ESPN_SWID) else {}
    try:
        r = requests.get(BASE_URL, params=params, cookies=cookies, headers=HEADERS, timeout=30)
        if r.status_code == 403:
            print(f"❌  HTTP 403 — ESPN is blocking the request.", file=sys.stderr)
            if not ESPN_S2 or not ESPN_SWID:
                print("   ESPN_S2 / ESPN_SWID secrets are missing. See README for setup.", file=sys.stderr)
            else:
                print("   Cookies may be expired. Re-copy espn_s2 and SWID from your browser.", file=sys.stderr)
            sys.exit(1)
        r.raise_for_status()
        return r.json()
