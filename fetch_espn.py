...
def fetch_current_season():
    ...
    def nc(key):
        ...
        # If all equal (no data yet), spread by roster depth as proxy
        all_eq = len(set(round(v,4) for v in vals)) <= 1
        if all_eq:
            if "Hit" in key or "hit" in key:
                vals = [raw_rs[tid]["hitDepth"] for tid in all_tids]
            else:
                vals = [raw_rs[tid]["pitchDepth"] for tid in all_tids]
