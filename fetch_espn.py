...
def main():
    ...
    # ESPN's current_week can tick forward before the scoring period ends.
    # If box_scores(current_week) returns nothing, fall back to current_week - 1.
    def get_active_week(league, week):
        try:
            boxes = league.box_scores(week)
            if boxes and any(getattr(b, 'home_team', None) for b in boxes):
                return week, boxes
        except Exception:
            pass
        # Try previous week
        if week > 1:
            try:
                boxes = league.box_scores(week - 1)
                if boxes and any(getattr(b, 'home_team', None) for b in boxes):
                    print(f"  ⚠️  Week {week} empty — falling back to week {week-1}")
                    return week - 1, boxes
            except Exception:
                pass
        return week, []

    scoring_week, prefetched_boxes = get_active_week(league, current_week)
    print(f"  📅  Scoring week: {scoring_week}")
    ...
    matchups_out = []
    try:
        box_scores = prefetched_boxes if prefetched_boxes else league.box_scores(scoring_week)
        print(f"  📊  Got {len(box_scores)} box scores for week {scoring_week}")

        for b in box_scores:
            ht, at = b.home_team, b.away_team
            hn = ht.team_name if ht else "BYE"
            an = at.team_name if at else "BYE"
            print(f"  Matchup: {hn} vs {an}  (cats: H={b.home_wins} A={b.away_wins})")

            # Get raw stats for each side
            home_raw = get_team_cats(ht, b, is_home=True)
            away_raw = get_team_cats(at, b, is_home=False)

            # Debug: print what we got
            print(f"    home stats: {dict(list(home_raw.items())[:5])}")
            print(f"    away stats: {dict(list(away_raw.items())[:5])}")

            home_cats, away_cats = resolve_cat_results(home_raw, away_raw)

            home_side = build_side(ht, b.home_wins, b.home_losses, b.home_ties, home_cats, team_map)
            away_side = build_side(at, b.away_wins, b.away_losses, b.away_ties, away_cats, team_map)
        ...
        print(f"  ⚠️   Box scores error: {e}", file=sys.stderr)
