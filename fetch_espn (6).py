...
# ── Stat extraction ───────────────────────────────────────────────────────────
def get_stats(player, projected=False):
    """Pull season stats (actual or projected) from espn-api player."""
    raw = {}
    try:
        stats = getattr(player, "stats", {}) or {}
        keys = (["10","01","102026","012026"] if projected
                else ["00","001","002026","0"])
        for key in keys:
            if key in stats:
                entry = stats[key]
                totals = entry.get("total", entry) if isinstance(entry, dict) else {}
                for cat in ALL_CATS + ["HLD"]:
                    v = totals.get(cat)
                    if v is not None:
                        raw[cat] = round(float(v), 3)
                if raw:
                    break
    except Exception:
        pass
...
def blended_stats(player, pw):
    """Return projection-blended stats dict for a player."""
    actual    = get_stats(player, projected=False)
    projected = get_stats(player, projected=True)
    if pw == 0.0 or not projected:
        return actual
    result = {}
    all_keys = set(actual) | set(projected)
    for cat in all_keys:
        a = actual.get(cat); p = projected.get(cat)
        v = blend(a, p, pw)
        if v is not None:
            result[cat] = round(float(v), 3)
    return result
...
def fetch_current_season():
    ...
    for t in league.teams:
        ...
        for player in getattr(t,"roster",[]):
            ...
            is_pitcher      = pos in PITCHER_POS or slot_id in ACTIVE_PITCH_SLOTS

            pd = {
                "name":name,"position":pos,
                "slot":SLOT_LABEL.get(slot_id,str(slot_id)),
                "injStatus":inj,"onBench":on_bench,"onIL":on_il,
                "isPitcher":is_pitcher,"stats":stats,
            }
...
def generate_narratives(rosters_out, power_rankings, legacy_teams, current_week):
    """Call Anthropic API to generate team identity narratives. Skips if no key."""
    ...
    updated = now_utc()
    narratives = {}
    pr_map = {p["name"]: p for p in power_rankings}
    legacy_map = {lt["name"]: lt for lt in legacy_teams}

    for team in rosters_out:
        name = team["name"]
        print(f"    → Generating narrative for {name} …")

        # Build player lists
        active_hitters  = [p for p in team["players"] if not p["onBench"] and not p["onIL"] and not p["isPitcher"]]
        active_pitchers = [p for p in team["players"] if not p["onBench"] and not p["onIL"] and p["isPitcher"]]

        def fmt_players(players, is_hit):
            lines = []
            cats = ["AVG","OPS","R","RBI","HR","SB"] if is_hit else ["ERA","WHIP","IP","K","QS","SV"]
            for p in players[:8]:
                stat_str = ", ".join(f"{c}:{p['stats'].get(c,'—')}" for c in cats if p['stats'].get(c) is not None)
                tier = p.get("tier","")
                lines.append(f"  {p['position']} {p['name']} [{tier}] — {stat_str}")
            return "
".join(lines) if lines else "  (none)"

        pr  = pr_map.get(name, {})
        leg = legacy_map.get(name, {})
        hist = leg.get("seasonHistory", [])
        hist_str = "
".join(f"  {s['season']}: {s['wins']}-{s['losses']} (Rank #{s['finalRank'] or '?'})" for s in hist[-5:]) if hist else "  No history available"

        prompt = f"""You are a sharp fantasy baseball analyst writing team profiles for "The League" — a 12-team H2H categories ESPN league running since 2013. Categories: R, RBI, HR, SB, K, AVG, OPS (hitting); IP, H, K, QS, ERA, WHIP, SV (pitching).

Team: {name}
2026 season: {pr.get('overallW',0)}-{pr.get('overallL',0)}, Power Rank #{pr.get('pwRank','?')} of 12, Composite Score: {pr.get('composite','?')}
All-time: {leg.get('allTimeW',0)}-{leg.get('allTimeL',0)} ({round(leg.get('winPct',0)*100,1)}% win rate)
Championships: {leg.get('components',{}).get('championships',0)}, Playoff apps: {leg.get('components',{}).get('playoffApps',0)}, Legacy Rank: #{leg.get('legacyRank','?')}
Starter Hit Score: {pr.get('starterHitScore',50):.0f}/100, Starter Pitch Score: {pr.get('starterPitScore',50):.0f}/100

Active Hitters:
{fmt_players(active_hitters, True)}

Active Pitchers:
{fmt_players(active_pitchers, False)}
...
{{
  "tagline": "A punchy 8-12 word motto capturing their identity and style",
  "identity_p1": "130-160 words: franchise story, what defines them in the league, historical performance pattern, reputation among opponents",
  "identity_p2": "130-160 words: current roster identity, call out specific players by name from the lists above, hitting approach and pitching philosophy",
  "identity_p3": "80-100 words: 2026 season context — where they stand, what needs to go right, what the season means",
  "outlook_emoji": "single most fitting emoji for their 2026 outlook",
  "outlook_headline": "Bold 8-12 word headline for their 2026 season",
  "outlook_body": "60-80 words: frank assessment of their 2026 chances and path"
}}"""
        ...
        try:
            ...
            narratives[name] = {**data, "team": name, "abbrev": team["abbrev"], "isMyTeam": team["isMyTeam"]}
            print(f"      ✅ Done")
        except Exception as e:
            print(f"      ⚠ Failed for {name}: {e}")
            narratives[name] = {
                "team": name, "abbrev": team["abbrev"], "isMyTeam": team["isMyTeam"],
                "tagline": "Building toward greatness.",
                "identity_p1": f"{name} is a competitor in The League, always looking to make moves and climb the standings.",
                "identity_p2": "A balanced roster with pieces in all the right places. Watch this squad.",
                "identity_p3": "The 2026 season is another chapter in an ongoing story.",
                "outlook_emoji": "⚾",
                "outlook_headline": "Ready to compete in 2026.",
                "outlook_body": "Every season is a new opportunity. This squad has the talent to make noise."
            }
        time.sleep(1.5)  # gentle rate limiting

    save("team_narratives.json", {"teams": narratives, "week": current_week, "updated": updated})
    return narratives
...
if __name__ == "__main__":
    ...
    current_league = fetch_current_season()
    franchise_data, all_standings, h2h_out = fetch_history()
    legacy_teams_out = compute_legacy(franchise_data, all_standings, h2h_out)
    # Generate AI narratives (requires ANTHROPIC_API_KEY secret)
    try:
        import json as _json
        with open("data/rosters.json") as rf: ros = _json.load(rf)["teams"]
        with open("data/power_rankings.json") as pf: pr  = _json.load(pf)["rankings"]
        with open("data/legacy_scores.json") as lf: leg = _json.load(lf)["teams"]
        current_week = _json.load(open("data/meta.json"))["currentWeek"]
        generate_narratives(ros, pr, leg, current_week)
    except Exception as e:
