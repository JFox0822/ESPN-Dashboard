...
def get_stats(player, projected=False):
    """
    Pull season stats from espn-api player object.
    ESPN stat key format: "{statSplitTypeId}{seasonId}"
      statSplitTypeId 0 = actual season total
      statSplitTypeId 1 = projected season total
    2026 keys: "002026" (actual), "012026" (projected)
    2025 keys: "002025" (last year actual) — used as fallback baseline
    """
    raw = {}
    try:
        ...
        if projected:
            candidate_keys = ["012026","12026","102026","10","1"]
            # Scan for any entry ESPN tagged with statSplitTypeId=1
            for k, v in stats.items():
                if isinstance(v, dict) and v.get("statSplitTypeId") == 1:
                    if k not in candidate_keys:
                        candidate_keys.insert(0, k)
        else:
            candidate_keys = ["002026","02026","002025","02025","00","0"]
            for k, v in stats.items():
                if isinstance(v, dict) and v.get("statSplitTypeId") == 0:
                    if k not in candidate_keys:
                        candidate_keys.insert(0, k)
        ...
        for key in candidate_keys:
            ...
            if not isinstance(entry, dict):
                continue
            totals = entry.get("total") or entry.get("stats") or entry
        ...
        # Broadest fallback: scan all entries, pick the one with most populated stats
        if not raw and stats:
            best_entry = None
            best_count = 0
            for k, v in stats.items():
                if not isinstance(v, dict): continue
                totals = v.get("total") or v.get("stats") or v
                if not isinstance(totals, dict): continue
                count = sum(1 for cat in ALL_CATS if totals.get(cat) not in (None, 0))
                if count > best_count:
                    best_count = count
                    best_entry = totals
            if best_entry and best_count >= 3:
                for cat in ALL_CATS + ["HLD"]:
                    v = best_entry.get(cat)
                    if v is not None and float(v) != 0:
                        raw[cat] = round(float(v), 3)
...
def get_prior_year_stats(player):
    """
    Pull last year (2025) actual stats as a projection baseline.
    Scales counting stats to a full 162-game season equivalent.
    Used when ESPN projections are unavailable.
    """
    raw = {}
    try:
        stats = getattr(player, "stats", {}) or {}
        # Try 2025 actual keys
        for key in ["002025", "02025", "00", "0"]:
            if key not in stats:
                continue
            entry = stats[key]
            if not isinstance(entry, dict):
...
# Track projection hit rate for diagnostics
_proj_hits = [0, 0, 0]   # [proj_found, prior_year_used, total]

def blended_stats(player, pw):
    """
    Return projection-blended stats for a player.
    Priority for the 'projected' component:
      1. ESPN 2026 projections
      2. 2025 actual stats as baseline (if projections unavailable)
      3. Current 2026 actuals only (if nothing else available)
    """
    actual    = get_stats(player, projected=False)
    projected = get_stats(player, projected=True)
    _proj_hits[2] += 1

    # Fallback: use prior year stats as projection baseline
    if not projected:
        projected = get_prior_year_stats(player)
        if projected:
            _proj_hits[1] += 1   # prior year used
    else:
        _proj_hits[0] += 1       # real projection used

    # No projection weight or nothing to blend with
